...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE TABLE IF NOT EXISTS email_logs (
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
      booking_id uuid REFERENCES bookings(id) ON DELETE CASCADE,
      recipient text NOT NULL,
      subject text NOT NULL,
    ...
    EXECUTE 'CREATE INDEX IF NOT EXISTS email_logs_booking_id_idx ON email_logs(booking_id)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS email_logs_email_type_idx ON email_logs(email_type)';
    
    EXECUTE 'ALTER TABLE email_logs ENABLE ROW LEVEL SECURITY';
    
    -- Create policy separately
    EXECUTE 'DROP POLICY IF EXISTS "Admins can view all email logs" ON email_logs';
    
    EXECUTE 'CREATE POLICY "Admins can view all email logs"
      ON email_logs
      FOR SELECT
      TO authenticated
      USING (
